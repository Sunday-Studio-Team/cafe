...
@export var horizontal_trajectory_curve: Curve
...
var bomb_bean_already_spawned := false
var screw_bean_already_spawned := false
...
func _physics_process(delta: float) -> void:
	...
	calculate_cup_x_velocity(left_right_input)
	adjust_cup_y_velocity() # bandaid; calling move_local_x causes the cup to vertically move down.
	#cup.position.y=1050
	cup.rotation_degrees = lerp(cup.rotation_degrees, left_right_input * 7, delta * 5)

	accuracy = beans_in_cup as float / NUM_BEANS_TO_SPAWN as float
...
func adjust_cup_y_velocity() -> void:
	#the cup should be at 1050
	var _offset: float = 1050.0 - cup.position.y
	#if offset is negative, push up.
	cup.velocity.y = _offset
	cup.move_local_y(cup.velocity.y)
...
func spawn_bean() -> void:
	if beans_spawned >= NUM_BEANS_TO_SPAWN:
		...
		Global.refill_minigame_accuracy = accuracy
		Events.minigame_end.emit()
		return

	#create a random int from 1-100
	var random_int: int
	var bean: RigidBody2D = null
...
func get_closest_machine_or_null():
	...
	if (all_machines.size() > 0):
		...
		for _machine in all_machines:
			var distance_to_this_machine = _player_global_position.distance_to(
				_machine.global_position
			)
			var distance_to_closest_machine = _player_global_position.distance_to(
				closest_machine.global_position
			)
...
func randomize_trajectory() -> void:
	#i want it to tend to not add ~0... i want it to add -450-200 ish
	...
	spawn_trajectory.y = clampf(spawn_trajectory.y, -650.0, -300.0)
