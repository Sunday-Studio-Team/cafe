...
func _unhandled_input(input_event: InputEvent) -> void:
	match _state:
		State.PLAYING:
			if input_event.is_echo() or not input_event.is_pressed():
				return
...
func _update_style_player_reply() -> void:
	...
	if _state == State.FINISHED:
		...
		styled_player_reply = styled_player_reply.insert(
			styled_player_reply.length(),
			past_words_correct_letters_bbcode_close,
		)
	...
	while (
		styled_player_reply[current_word_end_index] != " "
		and current_word_end_index < styled_player_reply.length() - 1
	):
	...
	if _state != State.PRE_START:
		styled_player_reply = styled_player_reply.insert(
			...
			past_words_correct_letters_bbcode_open,
		)
		offset += past_words_correct_letters_bbcode_open.length()

		styled_player_reply = styled_player_reply.insert(
			offset + past_words_correct_letters_bbcode_close_index,
			past_words_correct_letters_bbcode_close,
		)
		offset += past_words_correct_letters_bbcode_close.length()

		styled_player_reply = styled_player_reply.insert(
			offset + current_word_previous_letters_bbcode_open_index,
			current_word_previous_letters_bbcode_open,
		)
		offset += current_word_previous_letters_bbcode_open.length()

		styled_player_reply = styled_player_reply.insert(
			offset + current_word_previous_letters_bbcode_close_index,
			current_word_previous_letters_bbcode_close,
		)
		offset += current_word_previous_letters_bbcode_close.length()

		styled_player_reply = styled_player_reply.insert(
			offset + current_word_next_letter_bbcode_open_index,
			current_word_next_letter_bbcode_open,
		)
		offset += current_word_next_letter_bbcode_open.length()

		styled_player_reply = styled_player_reply.insert(
			offset + current_word_next_letter_bbcode_close_index,
		...
		if current_word_future_letters_bbcode_open_index <= current_word_future_letters_bbcode_close_index:
			styled_player_reply = styled_player_reply.insert(
				...
				current_word_future_letters_bbcode_open,
			)
			offset += current_word_future_letters_bbcode_open.length()

			styled_player_reply = styled_player_reply.insert(
				offset + current_word_future_letters_bbcode_close_index,
